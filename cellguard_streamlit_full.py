...
# Sidebar
with st.sidebar:
    st.image("https://via.placeholder.com/200x80/667eea/ffffff?text=CellGuard.AI", use_container_width=True)
...
if st.session_state.user_mode == 'driver':
    ...
    <div class="{status_class}">
        ...
        <h2 style="font-size: 3rem; margin: 0.5rem 0;">{status_text}</h2>
        <p style="font-size: 1.5rem; margin: 0;">{status_msg}</p>
        <h1 style="font-size: 5rem; margin: 1rem 0;">{health_score:.0f}</h1>
        <p style="font-size: 1.2rem; margin: 0;">Battery Health Score / 100</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if critical_high_alerts:
        ...
        for alert in critical_high_alerts:
            ...
            <div class="{alert_class}">
                <h3 style="margin: 0 0 0.5rem 0;">{emoji} {alert['title']}</h3>
                <p style="margin: 0.5rem 0;"><strong>What's happening:</strong> {alert['detail']}</p>
                <div style="background: rgba(255,255,255,0.7); padding: 0.75rem; border-radius: 8px; margin-top: 0.5rem;">
                    <p style="margin: 0;"><strong>✅ What to do:</strong> {alert['action']}</p>
                </div>
            </div>
            """, unsafe_allow_html=True)
    else:
...
# Technician Mode
    ...
    with col1:
        st.metric("Pack Voltage", f"{voltage:.2f}V", delta=f"{voltage-77.4:.2f}V vs nominal")
    ...
    with col3:
        st.metric("SOC", f"{soc:.1f}%", delta="Calibrated ✅")
    ...
    for alert in alerts:
        alert_class = f"alert-{alert['level']}"
        emoji = {'critical': '🔴', 'high': '🟠', 'medium': '🟡', 'low': '🔵'}[alert['level']]
        
        with st.expander(f"{emoji} [{alert['level'].upper()}] {alert['title']} - {alert['timestamp']}"):
    ...
    with tab1:
        fig_health = px.line(df_out, x='time', y='battery_health_score',
